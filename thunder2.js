...
function Thunder(options) {
  ...
  this.render = function (ctx) {
    ...
    ctx.closePath();
    ctx.shadowBlur = 0;
    const strength = Math.random() * 40 + 40;
...
(function () {
  ...
  input.addEventListener("input", (e) => {
    ...
    cb = setTimeout(() => {
      thunder = []; // clear existing lightning
      particles = [];
      text = new Text({
        copy: input.value,
      });
